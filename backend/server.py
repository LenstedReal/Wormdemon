...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Connection pooling ile optimize (TIMEOUT ARTTIRILDI)
        client = AsyncIOMotorClient(
            mongo_url,
            serverSelectionTimeoutMS=20000,  # 5s -> 20s (MongoDB Atlas için)
            connectTimeoutMS=15000,           # Bağlantı timeout eklendi
            socketTimeoutMS=30000,            # Socket timeout eklendi
            maxPoolSize=15,                   # 10 -> 15 (daha fazla eşzamanlılık)
            minPoolSize=2,                    # 1 -> 2 (hazır bağlantı)
            maxIdleTimeMS=60000,              # 45s -> 60s (daha az yeniden bağlanma)
            retryWrites=True,                 # Yazma hatalarını tekrar dene
            retryReads=True                   # Okuma hatalarını tekrar dene
        )
        db = client[db_name]
...
async def web_search(query: str) -> tuple[str, List[str]]:
    ...
    try:
        ...
        # SerpAPI ile gerçek arama (TIMEOUT EKLENDİ)
        params = {
            "q": query,
            "api_key": serpapi_key,
            "engine": "google",
